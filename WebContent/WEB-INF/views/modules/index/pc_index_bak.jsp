	...
	<script type="text/javascript">
		...
		function getProductData(offset,size){
			...
			$.get(url, param, function(res) {
				if(res.code==0){
					var picurl = "http://sh.sureserve.cn/upload/product_model/20190724200230639.png";
		...
		function getBatchData(productId){
			var url = '${pageContext.request.contextPath}/api/pcIndex/getBatchList';
			var param = {
				token: '${token}',
				productId: productId
	    	};
	    ...
	    function applyLabel(id,index){
	    	...
	    	layer.open({
    	        ...
    	        area: ['85%', '90%'],
    	        scrollbar: false, // 父页面 滚动条 禁止
    	        content: '${pageContext.request.contextPath}/api/pcLabel/applyLablePage?token=${token}&productId='+id,
    	        end:function (){
    	        	localStorage.clear();
    	        	var handle = $("#handle").val();
			        if ( handle && handle == '3' ) {
