layui.define(['jquery'], function (exports) {
  ...
  $.fn.step = function(options) {
    ...
    this.find(".step-header li").each(function (i, li) {
      if(i == 0){
        $(li).addClass("step-current").append('<a href="javascript:;" class="jump-steps" data-step="'+(i+1)+'">'+(i+1)+'</a>');
      } else if (i<opts.initStep){
        $(li).addClass("step-active").append('<a href="javascript:;" class="jump-steps" data-step="'+(i+1)+'"></a>');
      }else{
    ...
    this.goStep=function(page) {
	  ...
	  if(curPage<4){
		  ...
		  this.find(".step-list").eq(2).show();
	  }
     
      this.find(".step-header li").each(function (i, li) {
        var $li=$(li);
        $li.removeClass('step-current').removeClass('step-active');
        $li.find("a").html(i+1);
        if ((i+1)<page){
